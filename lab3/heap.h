...
Heap::Heap( const Heap & Heap1, const Heap & Heap2, int c ){ 
   ...
   A = new Pair[hCapacity];
   hSize = 0;
	for( int x = 0; x < Heap1.hSize; x++ ){
		insert(Heap1.A[x].element, Heap1.A[x].priority);
	}

	for( int y = 0; y < Heap2.hSize; y++ ){
		insert(Heap2.A[y].element, Heap2.A[y].priority);
	}  
...
void Heap::trickleUp(int i){
   ...
   if( A[i].priority < A[minParent].priority ){
	swap(i, minParent);
	trickleUp(minParent);
	}
...
int Heap::extractMin(){
   ...
   int placeholder = peekMin();
	A[0] = A[hSize-1];
	hSize = hSize-1;
	trickleDown(0);
   return placeholder;// Complete this.
}
...
void Heap::trickleDown(int i){
   ...
   if( A[minPriority].priority > A[minLeft].priority ){
	minPriority = minLeft;
	}

   if( (minRight < hSize) && (A[minPriority].priority > A[minRight].priority) ){
	minPriority = minRight;
	}
  
   if( minPriority != i ){
	swap(i, minPriority);
	trickleDown(minPriority);
	}
	// Complete this.
}
